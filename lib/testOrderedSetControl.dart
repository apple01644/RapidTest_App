...
class TestOrderedSetControl extends StatefulWidget {
  Function onAnswered;
  TestOrderedSet testData;
  int testSeq;

  TestOrderedSetControl({Key key, this.onAnswered, this.testData, this.testSeq})
      : super(key: key);
  ...
  @override
  createState() => TestOrderedSetControlState();
}

class TestOrderedSetControlState extends State<TestOrderedSetControl> {
  ...
  @override
  didUpdateWidget(covariant TestOrderedSetControl oldWidget) {
      ...
      buttonList.add(buildChooseButton(text, (idx) {
        setState(() {
          if (widget.answeredChooseButtonData.length <
            ...
            if (widget
                    .testData.answers[widget.answeredChooseButtonData.length] ==
                widget.chooseButtonData[idx]) {
              ...
              for (int x = 0; x < widget.chooseButtonStates.length; ++x)
                if (widget.chooseButtonStates[x] == TestAnswerButtonState.Wrong)
                  widget.chooseButtonStates[x] = TestAnswerButtonState.Normal;
            } else
